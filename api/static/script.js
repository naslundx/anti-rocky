...
const feasibilityDescription = document.querySelector("#mission-box p");
const missionBox = document.querySelector("#mission-box div:last-of-type");
...
async function updateInfo(key) {
  ...
  await fetch(`/api/objects/${key}/missions/?end_date=${closestDistanceDate}`)
    .then((response) => response.json())
    .then((missionData) => {
      ...
      if (missions === 0) {
        missionInnerHTML = `<span style="color: var(--muted);">No rendezvous missions possible for this encounter.</span>`;
      }
